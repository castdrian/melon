...
export class InfoCommand extends Command {
	...
	try {
		...
		const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
			new ButtonBuilder().setLabel('Contact').setStyle(ButtonStyle.Link).setURL('discord://-/users/224617799434108928'),
			...
			new ButtonBuilder().setLabel('Add to Server').setStyle(ButtonStyle.Link).setURL('https://github.com/castdrian/melon').setDisabled(true),
		);

		await interaction.reply({ embeds: [embed], components: [row] });
		await interaction.followUp({ content: '*This application is currently in its invite-only stage, if you wish to add it to your guild please contact [@castdrian](<https://discord.com/users/224617799434108928>) directly.*', ephemeral: true });
	} catch (ex) {
